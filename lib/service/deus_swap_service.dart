...
class SwapService {
  ...
  SwapService(String account, int chainId)
      : this.account = account,
        ...
        this.addrs = jsonDecode(
            File("lib/data_source/addresses.json").readAsStringSync()),
        ...
        this.infuraWeb3 = Web3Client(
            'http://' + _getInfuraURL(chainId), http.Client(),
            socketConnector: () => IOWebSocketChannel.connect(
                    'wss://' + _getInfuraURL(chainId))
                .cast<
                    String>()) //new Web3(new Web3.providers.WebsocketProvider('wss://' + this.INFURA_URL));
  {
    this.AutomaticMarketMakerContract = DeployedContract(
        abis["amm"],
        this.getAddr(
            "amm")); // this.infuraWeb3.eth.Contract(abis["amm"], this.getAddr("amm"));
    this.StaticSalePrice = DeployedContract(
        abis["sps"],
        this.getAddr(
            "sps")); //  this.infuraWeb3.eth.Contract(abis["sps"], this.getAddr("sps"));
    this.DeusSwapContract = DeployedContract(
        abis["deus_swap_contract"],
        this.getAddr(
            "deus_swap_contract")); // this.infuraWeb3.eth.Contract(abis["deus_swap_contract"], this.getAddr("deus_swap_contract"));
    this.uniswapRouter = DeployedContract(
        abis["uniswap_router"],
        this.getAddr(
            "uniswap_router")); // this.infuraWeb3.eth.Contract(abis["uniswap_router"], this.getAddr("uniswap_router"));
  }

  static _getInfuraURL(int chainId) =>
  ...
  bool _isDeus(String element) =>
      EthereumAddress.fromHex(element) == this.getTokenAddr("deus");

  bool checkWallet() => this.account != null && this.chainId != null;

  EthereumAddress getAddr(String tokenName) =>
      EthereumAddress.fromHex(addrs[tokenName][this.chainId.toString()]);

  EthereumAddress getTokenAddr(String tokenName) => EthereumAddress.fromHex(
      addrs["token"][tokenName][this.chainId.toString()]);
  ...
  String _getWei(dynamic number, [token = "eth"]) {
    final int maxDigit = SwapService.TokensMaxDigit.containsKey(token)
        ? SwapService.TokensMaxDigit[token]
        : 18;
    final String value = number.runtimeType == String
        ? double.parse(number).toStringAsFixed(18)
        : number.toStringAsFixed(18);
    String ans = EtherAmount.fromUnitAndValue(EtherUnit.ether, value)
  ...
  String _fromWei(value, token) {
    var max = SwapService.TokensMaxDigit.containsKey(token)
        ? SwapService.TokensMaxDigit[token]
        : 18;
    var ans;
  ...
  getTokenBalance(String tokenName) {
    ...
    final TokenContract =
        DeployedContract(abis["token"], this.getTokenAddr(tokenName));
  ...
  getAllowances(token) {
    ...
    return TokenContract.methods
        .allowance(account, this.getAddr("deus_swap_contract"))
        .call()
        .then((amount) {
  ...
  swapTokens(fromToken, toToken, tokenAmount, listener) {
    ...
    var metamaskWeb3 = new Web3(Web3.givenProvider);
    const DeusSwapContract = new metamaskWeb3.eth.Contract(
        abis["deus_swap_contract"], this.getAddr("deus_swap_contract"));
    ...
    if (fromToken == 'coinbase') {
      if (toToken == 'deus') {
        return DeusSwapContract.methods
            .swapTokensForTokens(
                this._getWei(tokenAmount, fromToken), 8, [], [])
            .send({from: this.account})
            .once('transactionHash', () => listener("transactionHash"))
            .once('receipt', () => listener("receipt"))
            .once('error', () => listener("error"));
        ...
        if (path[2] == this.getTokenAddr("weth")) {
          ...
          return DeusSwapContract.methods
              .swapTokensForTokens(this._getWei(tokenAmount, fromToken), 4,
                  path1, []) // change type
              .send({from: this.account})
              .once('transactionHash', () => listener("transactionHash"))
              .once('receipt', () => listener("receipt"))
          ...
          return DeusSwapContract.methods
              .swapTokensForTokens(this._getWei(tokenAmount, fromToken), 3,
                  path1, []) // change type
              .send({from: this.account})
              .once('transactionHash', () => listener("transactionHash"))
              .once('receipt', () => listener("receipt"))
      ...
      if (fromToken == 'deus') {
        return DeusSwapContract.methods
            .swapTokensForTokens(
                this._getWei(tokenAmount, fromToken), 7, [], [])
            .send({from: this.account})
            .once('transactionHash', () => listener("transactionHash"))
            .once('receipt', () => listener("receipt"))
            .once('error', () => listener("error"));
        ...
        if (path[path.length - 3] == this.getTokenAddr("weth")) {
          ...
          return DeusSwapContract.methods
              .swapTokensForTokens(
                  this._getWei(tokenAmount, fromToken), 5, path1, [])
              .send({from: this.account})
              .once('transactionHash', () => listener("transactionHash"))
              .once('receipt', () => listener("receipt"))
              .once('error', () => listener("error"));
          ...
          return DeusSwapContract.methods
              .swapTokensForTokens(
                  this._getWei(tokenAmount, fromToken), 6, path1, [])
              .send({from: this.account})
              .once('transactionHash', () => listener("transactionHash"))
              .once('receipt', () => listener("receipt"))
              .once('error', () => listener("error"));
      ...
      if (fromToken == 'eth') {
        ...
        if (indexOfDeus != -1) {
          if (indexOfDeus < path.length - 1) {
            if (path[indexOfDeus + 1] == this.getTokenAddr("weth")) {
              ...
              return DeusSwapContract.methods
                  .swapTokensForTokens(
                      this._getWei(tokenAmount, fromToken), 1, path1, path2)
                  .send({from: this.account})
                  .once('transactionHash', () => listener("transactionHash"))
                  .once('receipt', () => listener("receipt"))
                  .once('error', () => listener("error"));
          ...
          if (indexOfDeus > 0) {
            if (path[indexOfDeus - 1] == this.getTokenAddr("weth")) {
              ...
              return DeusSwapContract.methods
                  .swapTokensForTokens(
                      this._getWei(tokenAmount, fromToken), 0, path1, path2)
                  .send({from: this.account})
                  .once('transactionHash', () => listener("transactionHash"))
                  .once('receipt', () => listener("receipt"))
                  .once('error', () => listener("error"));
        ...
        return DeusSwapContract.methods
            .swapTokensForTokens(
                this._getWei(tokenAmount, fromToken), 2, path, [])
            .send({from: this.account})
            .once('transactionHash', () => listener("transactionHash"))
            .once('receipt', () => listener("receipt"))
            .once('error', () => listener("error"));
  ...
  withdrawPayment(listener) {
    var metamaskWeb3 = new Web3(Web3.givenProvider);
    const AutomaticMarketMakerContract =
      ...
      if (path[1] == this.getTokenAddr("weth")) {
          ...
          return this
            ...
            if (path.length < 2) {
              ...
              return this
                  .uniswapRouter
                  .methods
                  .getAmountsOut(etherAmount, path)
                  .call()
                  .then((amountsOut) {
                return this
                    ._fromWei(amountsOut[amountsOut.length - 1], toToken);
              });
            }
          });
        });
        ...
        return this
          ...
          return this
              .uniswapRouter
              .methods
              .getAmountsOut(etherAmount[0], path)
              .call()
              .then((amountsOut) {
            return this._fromWei(amountsOut[amountsOut.length - 1], toToken);
          });
          ...
          return this
            ...
            return this
              ...
              return this
                  .StaticSalePrice
                  .methods
                  .calculatePurchaseReturn(tokenAmount)
                  .call()
                  .then((amountOut) {
              ...
              return this
                ...
                return this
                    ...
                    .calculatePurchaseReturn(
                        amountsOut2[amountsOut2.length - 1])
                    .call()
                    .then((tokenAmount) {
                  if (path2.length > 1) {
                    return this
                        .uniswapRouter
                        .methods
                        .getAmountsOut(tokenAmount, path2)
                        .call()
                        .then((amountsOut) {
                      return this
                          ._fromWei(amountsOut[amountsOut.length - 1], toToken);
  ...
  getAllowancesStocks() {
    ...
    return TokenContract.methods
        .allowance(account, this.getAddr("stocks_contract"))
        .call()
        .then((amount) {
  ...
  buyStock(stockAddr, amount, blockNo, v, r, s, price, fee, listener) {
    ...
    var metamaskWeb3 = new Web3(Web3.givenProvider);
    const StocksContract = new metamaskWeb3.eth.Contract(
        abis["stocks_contract"], this.getAddr("stocks_contract"));
